...
export function HomeScreen() {
  ...
  const [exerciseModalData, setExerciseModalData] =
    useState<ExerciseModalData | null>(null);
  const [seminarRegistered, setSeminarRegistered] = useState(false);
  const [seminarConfirmOpen, setSeminarConfirmOpen] = useState(false);

  const [seminarMaterialOpen, setSeminarMaterialOpen] = useState(false);
  const nextSeminar = {
    topic: "Seminario de Psicoeducación",
    date: "Junio 16, 2026",
  ...
  return (
    ...
    >
      ...
      />
      <ConfirmModal
        visible={seminarConfirmOpen}
        title="Confirmar inscripción"
        message={`¿Estás seguro que deseás inscribirte a "${nextSeminar.topic}" el ${nextSeminar.date} en el horario ${nextSeminar.time} con ${nextSeminar.professional}?`}
        confirmLabel="Sí, inscribirme"
        cancelLabel="Cancelar"
        iconName="user-check"
        onCancel={() => setSeminarConfirmOpen(false)}
        onConfirm={() => {
          setSeminarConfirmOpen(false);
          setSeminarRegistered(true);
        }}
      />

      <ComingSoonModal
        visible={seminarMaterialOpen}
        onClose={() => setSeminarMaterialOpen(false)}
        iconName="file-text"
